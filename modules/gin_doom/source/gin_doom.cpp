...
void Doom::run()
{
    ...
    // Wire this instance's audio engine into its data_t so the SFX/music C
    // callbacks (which now take data_t) can reach it, and give the engine the
    // data_t so it can find the music state when it renders.
