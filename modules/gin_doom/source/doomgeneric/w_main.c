...
boolean W_ParseCommandLine(data_t* data)
{
    ...
    if (p > 0)
    {
        for (p = p + 1; p<data->myargc && data->myargv[p][0] != '-'; ++p)
        {
            char *filename;

            modifiedgame = true;

            filename = D_TryFindWADByName(data->myargv[p]);
    ...
    p = M_CheckParmWithArgs (data, "-file", 1);
    if (p)
    {
	// the parms after p are wadfile/lump names,
	// until end of parms or another - preceded parm
	modifiedgame = true;            // homebrew levels
	while (++p != data->myargc && data->myargv[p][0] != '-')
        {
            char *filename;
