...
void T_VerticalDoor (data_t* data, vldoor_t* door)
{
	...
	if (!--door->topcountdown)
	{
		...
		break;
	      ...
	      case vld_normal:
		door->direction = -1; // time to go back down
		S_StartSound(data, &door->sector->soundorg, sfx_dorcls);
		break;
		
	      case vld_close30ThenOpen:
		door->direction = 1;
		S_StartSound(data, &door->sector->soundorg, sfx_doropn);
		break;
  ...
  mobj_t*	thing )
{
    ...
    return EV_DoDoor(data, line,type);
}
  ...
  vldoor_e	type )
{
	...
	// new door thinker
	rtn = 1;
	door = Z_Malloc(data, sizeof(*door), PU_LEVSPEC, 0);
	...
	switch(type)
	{
	  ...
	  case vld_blazeOpen:
		...
		S_StartSound(data, &door->sector->soundorg, sfx_bdopn);
	    break;
  ...
  mobj_t*	thing )
{
	...
	S_StartSound(data, &sec->soundorg,sfx_bdopn);
	break;
	
      case 1:	// NORMAL DOOR SOUND
      case 31:
	S_StartSound(data, &sec->soundorg,sfx_doropn);
	break;
...
void P_InitSlidingDoorFrames(void)
{
	...
	f1 = R_TextureNumForName(data, slideFrameNames[i].frontFrame1);
	f2 = R_TextureNumForName(data, slideFrameNames[i].frontFrame2);
	f3 = R_TextureNumForName(data, slideFrameNames[i].frontFrame3);
	f4 = R_TextureNumForName(data, slideFrameNames[i].frontFrame4);
	...
	f1 = R_TextureNumForName(data, slideFrameNames[i].backFrame1);
	f2 = R_TextureNumForName(data, slideFrameNames[i].backFrame2);
	f3 = R_TextureNumForName(data, slideFrameNames[i].backFrame3);
	f4 = R_TextureNumForName(data, slideFrameNames[i].backFrame4);
...
void T_SlidingDoor (data_t* data, slidedoor_t* door)
{
    switch(door->status)
    {
      case sd_opening:
  ...
  mobj_t*	thing )
{
	...
	if (door->whichDoorIndex < 0)
	    I_Error (NULL, "EV_SlidingDoor: Can't use texture for sliding door!");
