//
// Copyright(C) 1993-1996 Id Software, Inc.
// Copyright(C) 2005-2014 Simon Howard
//
// This program is free software; you can redistribute it and/or
// modify it under the terms of the GNU General Public License
// as published by the Free Software Foundation; either version 2
// of the License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// DESCRIPTION:  Heads-up displays
//


#include <ctype.h>

#include "doomdef.h"
#include "doomkeys.h"

#include "z_zone.h"

#include "deh_main.h"
#include "i_swap.h"
#include "i_video.h"

#include "hu_stuff.h"
#include "hu_lib.h"
#include "m_controls.h"
#include "m_misc.h"
#include "w_wad.h"

#include "s_sound.h"

#include "doomstat.h"

// Data.
#include "dstrings.h"
#include "sounds.h"

//
// Locally used constants, shortcuts.
//
#define HU_TITLE	(mapnames[(data->gameepisode-1)*9+data->gamemap-1])
#define HU_TITLE2	(mapnames_commercial[data->gamemap-1])
#define HU_TITLEP	(mapnames_commercial[data->gamemap-1 + 32])
#define HU_TITLET	(mapnames_commercial[data->gamemap-1 + 64])
#define HU_TITLE_CHEX   (mapnames[data->gamemap - 1])
#define HU_TITLEHEIGHT	1
#define HU_TITLEX	0
#define HU_TITLEY	(167 - SHORT(hu_font[0]->height))

#define HU_INPUTTOGGLE	't'
#define HU_INPUTX	HU_MSGX
#define HU_INPUTY	(HU_MSGY + HU_MSGHEIGHT*(SHORT(hu_font[0]->height) +1))
#define HU_INPUTWIDTH	64
#define HU_INPUTHEIGHT	1



char *chat_macros[10] =
{
    HUSTR_CHATMACRO0,
    HUSTR_CHATMACRO1,
    HUSTR_CHATMACRO2,
    HUSTR_CHATMACRO3,
    HUSTR_CHATMACRO4,
    HUSTR_CHATMACRO5,
    HUSTR_CHATMACRO6,
    HUSTR_CHATMACRO7,
    HUSTR_CHATMACRO8,
    HUSTR_CHATMACRO9
};

char*	player_names[] =
{
    HUSTR_PLRGREEN,
    HUSTR_PLRINDIGO,
    HUSTR_PLRBROWN,
    HUSTR_PLRRED
};

char			chat_char; // remove later.
patch_t*		hu_font[HU_FONTSIZE];





//
// Builtin map names.
// The actual names can be found in DStrings.h.
//

char*	mapnames[] =	// DOOM shareware/registered/retail (Ultimate) names.
{

    HUSTR_E1M1,
    HUSTR_E1M2,
    HUSTR_E1M3,
    HUSTR_E1M4,
    HUSTR_E1M5,
    HUSTR_E1M6,
    HUSTR_E1M7,
    HUSTR_E1M8,
    HUSTR_E1M9,

    HUSTR_E2M1,
    HUSTR_E2M2,
    HUSTR_E2M3,
    HUSTR_E2M4,
    HUSTR_E2M5,
    HUSTR_E2M6,
    HUSTR_E2M7,
    HUSTR_E2M8,
    HUSTR_E2M9,

    HUSTR_E3M1,
    HUSTR_E3M2,
    HUSTR_E3M3,
    HUSTR_E3M4,
    HUSTR_E3M5,
    HUSTR_E3M6,
    HUSTR_E3M7,
    HUSTR_E3M8,
    HUSTR_E3M9,

    HUSTR_E4M1,
    HUSTR_E4M2,
    HUSTR_E4M3,
    HUSTR_E4M4,
    HUSTR_E4M5,
    HUSTR_E4M6,
    HUSTR_E4M7,
    HUSTR_E4M8,
    HUSTR_E4M9,

    "NEWLEVEL",
    "NEWLEVEL",
    "NEWLEVEL",
    "NEWLEVEL",
    "NEWLEVEL",
    "NEWLEVEL",
    "NEWLEVEL",
    "NEWLEVEL",
    "NEWLEVEL"
};

// List of names for levels in commercial IWADs
// (doom2.wad, plutonia.wad, tnt.wad).  These are stored in a
// single large array; WADs like pl2.wad have a MAP33, and rely on
// the layout in the Vanilla executable, where it is possible to
// overflow the end of one array into the next.

char *mapnames_commercial[] =
{
    // DOOM 2 map names.

    HUSTR_1,
    HUSTR_2,
    HUSTR_3,
    HUSTR_4,
    HUSTR_5,
    HUSTR_6,
    HUSTR_7,
    HUSTR_8,
    HUSTR_9,
    HUSTR_10,
    HUSTR_11,
	
    HUSTR_12,
    HUSTR_13,
    HUSTR_14,
    HUSTR_15,
    HUSTR_16,
    HUSTR_17,
    HUSTR_18,
    HUSTR_19,
    HUSTR_20,
	
    HUSTR_21,
    HUSTR_22,
    HUSTR_23,
    HUSTR_24,
    HUSTR_25,
    HUSTR_26,
    HUSTR_27,
    HUSTR_28,
    HUSTR_29,
    HUSTR_30,
    HUSTR_31,
    HUSTR_32,

    // Plutonia WAD map names.

    PHUSTR_1,
    PHUSTR_2,
    PHUSTR_3,
    PHUSTR_4,
    PHUSTR_5,
    PHUSTR_6,
    PHUSTR_7,
    PHUSTR_8,
    PHUSTR_9,
    PHUSTR_10,
    PHUSTR_11,
	
    PHUSTR_12,
    PHUSTR_13,
    PHUSTR_14,
    PHUSTR_15,
    PHUSTR_16,
    PHUSTR_17,
    PHUSTR_18,
    PHUSTR_19,
    PHUSTR_20,
	
    PHUSTR_21,
    PHUSTR_22,
    PHUSTR_23,
    PHUSTR_24,
    PHUSTR_25,
    PHUSTR_26,
    PHUSTR_27,
    PHUSTR_28,
    PHUSTR_29,
    PHUSTR_30,
    PHUSTR_31,
    PHUSTR_32,
    
    // TNT WAD map names.

    THUSTR_1,
    THUSTR_2,
    THUSTR_3,
    THUSTR_4,
    THUSTR_5,
    THUSTR_6,
    THUSTR_7,
    THUSTR_8,
    THUSTR_9,
    THUSTR_10,
    THUSTR_11,
	
    THUSTR_12,
    THUSTR_13,
    THUSTR_14,
    THUSTR_15,
    THUSTR_16,
    THUSTR_17,
    THUSTR_18,
    THUSTR_19,
    THUSTR_20,
	
    THUSTR_21,
    THUSTR_22,
    THUSTR_23,
    THUSTR_24,
    THUSTR_25,
    THUSTR_26,
    THUSTR_27,
    THUSTR_28,
    THUSTR_29,
    THUSTR_30,
    THUSTR_31,
    THUSTR_32
};

void HU_Init(data_t* data)
{

    int		i;
    int		j;
    char	buffer[9];

    // load the heads-up font
    j = HU_FONTSTART;
    for (i=0;i<HU_FONTSIZE;i++)
    {
	DEH_snprintf(buffer, 9, "STCFN%.3d", j++);
	hu_font[i] = (patch_t *) W_CacheLumpName(data, buffer, PU_STATIC);
    }

}

void HU_Stop(data_t* data)
{
    data->hu_headsupactive = false;
}

void HU_Start(data_t* data)
{

    int		i;
    char*	s;

    if (data->hu_headsupactive)
	HU_Stop(data);

    data->hu_plr = &data->players[data->consoleplayer];
    data->hu_message_on = false;
    data->hu_message_dontfuckwithme = false;
    data->hu_message_nottobefuckedwith = false;
    data->hu_chat_on = false;

    // create the message widget
    HUlib_initSText(&data->hu_w_message,
		    HU_MSGX, HU_MSGY, HU_MSGHEIGHT,
		    hu_font,
		    HU_FONTSTART, &data->hu_message_on);

    // create the map title widget
    HUlib_initTextLine(&data->hu_w_title,
		       HU_TITLEX, HU_TITLEY,
		       hu_font,
		       HU_FONTSTART);
    
    switch ( logical_gamemission )
    {
      case doom:
	s = HU_TITLE;
	break;
      case doom2:
	 s = HU_TITLE2;
	 break;
      case pack_plut:
	s = HU_TITLEP;
	break;
      case pack_tnt:
	s = HU_TITLET;
	break;
      default:
         s = "Unknown level";
         break;
    }

    // Chex.exe always uses the episode 1 level title
    // eg. E2M1 gives the title for E1M1

    if (data->gameversion == exe_chex)
    {
        s = HU_TITLE_CHEX;
    }

    // dehacked substitution to get modified level name

    s = DEH_String(s);
    
    while (*s)
	HUlib_addCharToTextLine(&data->hu_w_title, *(s++));

    // create the chat widget
    HUlib_initIText(&data->hu_w_chat,
		    HU_INPUTX, HU_INPUTY,
		    hu_font,
		    HU_FONTSTART, &data->hu_chat_on);

    // create the inputbuffer widgets
    for (i=0 ; i<MAXPLAYERS ; i++)
	HUlib_initIText(&data->hu_w_inputbuffer[i], 0, 0, 0, 0, &data->hu_always_off);

    data->hu_headsupactive = true;

}

void HU_Drawer(data_t* data)
{

    HUlib_drawSText(data, &data->hu_w_message);
    HUlib_drawIText(data, &data->hu_w_chat);
    if (data->automapactive)
	HUlib_drawTextLine(data, &data->hu_w_title, false);

}

void HU_Erase(data_t* data)
{

    HUlib_eraseSText(data, &data->hu_w_message);
    HUlib_eraseIText(data, &data->hu_w_chat);
    HUlib_eraseTextLine(data, &data->hu_w_title);

}

void HU_Ticker(data_t* data)
{

    int i, rc;
    char c;

    // tick down message counter if message is up
    if (data->hu_message_counter && !--data->hu_message_counter)
    {
	data->hu_message_on = false;
	data->hu_message_nottobefuckedwith = false;
    }

    if (data->mn_showMessages || data->hu_message_dontfuckwithme)
    {

	// display message if necessary
	if ((data->hu_plr->message && !data->hu_message_nottobefuckedwith)
	    || (data->hu_plr->message && data->hu_message_dontfuckwithme))
	{
	    HUlib_addMessageToSText(&data->hu_w_message, 0, data->hu_plr->message);
	    data->hu_plr->message = 0;
	    data->hu_message_on = true;
	    data->hu_message_counter = HU_MSGTIMEOUT;
	    data->hu_message_nottobefuckedwith = data->hu_message_dontfuckwithme;
	    data->hu_message_dontfuckwithme = 0;
	}

    } // else data->hu_message_on = false;

    // check for incoming chat characters
    if (data->netgame)
    {
	for (i=0 ; i<MAXPLAYERS; i++)
	{
	    if (!data->playeringame[i])
		continue;
	    if (i != data->consoleplayer
		&& (c = data->players[i].cmd.chatchar))
	    {
		if (c <= HU_BROADCAST)
		    data->hu_chat_dest[i] = c;
		else
		{
		    rc = HUlib_keyInIText(&data->hu_w_inputbuffer[i], c);
		    if (rc && c == KEY_ENTER)
		    {
			if (data->hu_w_inputbuffer[i].l.len
			    && (data->hu_chat_dest[i] == data->consoleplayer+1
				|| data->hu_chat_dest[i] == HU_BROADCAST))
			{
			    HUlib_addMessageToSText(&data->hu_w_message,
						    DEH_String(player_names[i]),
						    data->hu_w_inputbuffer[i].l.l);
			    
			    data->hu_message_nottobefuckedwith = true;
			    data->hu_message_on = true;
			    data->hu_message_counter = HU_MSGTIMEOUT;
			    if ( data->gamemode == commercial )
			      S_StartSound(data, 0, sfx_radio);
			    else
			      S_StartSound(data, 0, sfx_tink);
			}
			HUlib_resetIText(&data->hu_w_inputbuffer[i]);
		    }
		}
		data->players[i].cmd.chatchar = 0;
	    }
	}
    }

}

#define QUEUESIZE		128



void HU_queueChatChar(data_t* data, char c)
{
    if (((data->hu_head + 1) & (QUEUESIZE-1)) == data->hu_tail)
    {
	data->hu_plr->message = DEH_String(HUSTR_MSGU);
    }
    else
    {
	data->hu_chatchars[data->hu_head] = c;
	data->hu_head = (data->hu_head + 1) & (QUEUESIZE-1);
    }
}

char HU_dequeueChatChar(data_t* data)
{
    char c;

    if (data->hu_head != data->hu_tail)
    {
	c = data->hu_chatchars[data->hu_tail];
	data->hu_tail = (data->hu_tail + 1) & (QUEUESIZE-1);
    }
    else
    {
	c = 0;
    }

    return c;
}

boolean HU_Responder(data_t* data, event_t *ev)
{

    char*		macromessage;
    boolean		eatkey = false;
    unsigned char 	c;
    int			i;
    int			numplayers;
    

    numplayers = 0;
    for (i=0 ; i<MAXPLAYERS ; i++)
	numplayers += data->playeringame[i];

    if (ev->data1 == KEY_RSHIFT)
    {
	return false;
    }
    else if (ev->data1 == KEY_RALT || ev->data1 == KEY_LALT)
    {
	data->hu_altdown = ev->type == ev_keydown;
	return false;
    }

    if (ev->type != ev_keydown)
	return false;

    if (!data->hu_chat_on)
    {
	if (ev->data1 == key_message_refresh)
	{
	    data->hu_message_on = true;
	    data->hu_message_counter = HU_MSGTIMEOUT;
	    eatkey = true;
	}
	else if (data->netgame && ev->data2 == key_multi_msg)
	{
	    eatkey = data->hu_chat_on = true;
	    HUlib_resetIText(&data->hu_w_chat);
	    HU_queueChatChar(data, HU_BROADCAST);
	}
	else if (data->netgame && numplayers > 2)
	{
	    for (i=0; i<MAXPLAYERS ; i++)
	    {
		if (ev->data2 == key_multi_msgplayer[i])
		{
		    if (data->playeringame[i] && i!=data->consoleplayer)
		    {
			eatkey = data->hu_chat_on = true;
			HUlib_resetIText(&data->hu_w_chat);
			HU_queueChatChar(data, i+1);
			break;
		    }
		    else if (i == data->consoleplayer)
		    {
			data->hu_num_nobrainers++;
			if (data->hu_num_nobrainers < 3)
			    data->hu_plr->message = DEH_String(HUSTR_TALKTOSELF1);
			else if (data->hu_num_nobrainers < 6)
			    data->hu_plr->message = DEH_String(HUSTR_TALKTOSELF2);
			else if (data->hu_num_nobrainers < 9)
			    data->hu_plr->message = DEH_String(HUSTR_TALKTOSELF3);
			else if (data->hu_num_nobrainers < 32)
			    data->hu_plr->message = DEH_String(HUSTR_TALKTOSELF4);
			else
			    data->hu_plr->message = DEH_String(HUSTR_TALKTOSELF5);
		    }
		}
	    }
	}
    }
    else
    {
	// send a macro
	if (data->hu_altdown)
	{
	    c = ev->data1 - '0';
	    if (c > 9)
		return false;
	    // fprintf(stderr, "got here\n");
	    macromessage = chat_macros[c];
	    
	    // kill last message with a '\n'
	    HU_queueChatChar(data, KEY_ENTER); // DEBUG!!!
	    
	    // send the macro message
	    while (*macromessage)
		HU_queueChatChar(data, *macromessage++);
	    HU_queueChatChar(data, KEY_ENTER);
	    
            // leave chat mode and notify that it was sent
            data->hu_chat_on = false;
            M_StringCopy(data->hu_lastmessage, chat_macros[c], sizeof(data->hu_lastmessage));
            data->hu_plr->message = data->hu_lastmessage;
            eatkey = true;
	}
	else
	{
            c = ev->data2;

	    eatkey = HUlib_keyInIText(&data->hu_w_chat, c);
	    if (eatkey)
	    {
		// static unsigned char buf[20]; // DEBUG
		HU_queueChatChar(data, c);
		
		// M_snprintf(buf, sizeof(buf), "KEY: %d => %d", ev->data1, c);
		//        data->hu_plr->message = buf;
	    }
	    if (c == KEY_ENTER)
	    {
		data->hu_chat_on = false;
                if (data->hu_w_chat.l.len)
                {
                    M_StringCopy(data->hu_lastmessage, data->hu_w_chat.l.l, sizeof(data->hu_lastmessage));
                    data->hu_plr->message = data->hu_lastmessage;
                }
	    }
	    else if (c == KEY_ESCAPE)
		data->hu_chat_on = false;
	}
    }

    return eatkey;

}
