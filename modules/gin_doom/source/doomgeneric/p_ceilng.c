...
void T_MoveCeiling (data_t* data, ceiling_t* ceiling)
{
	...
	break;
      case 1:
	// UP
	res = T_MovePlane(data, ceiling->sector,
			  ceiling->speed,
			  ceiling->topheight,
	...
	if (!(data->leveltime&7))
	{
		...
		break;
	      default:
		S_StartSound(data, &ceiling->sector->soundorg, sfx_stnmov);
		// ?
		break;
	    }
	}
	...
	if (res == pastdest)
	{
	    ...
	    {
	      case raiseToHighest:
		P_RemoveActiveCeiling(data, ceiling);
		break;
		
	      case silentCrushAndRaise:
		S_StartSound(data, &ceiling->sector->soundorg, sfx_pstop);
  ...
  ceiling_e	type )
{
    ...
    switch(type)
    {
      ...
      case silentCrushAndRaise:
      case crushAndRaise:
	P_ActivateInStasisCeiling(data, line);
      default:
	break;
    }
	...
	// new door thinker
	rtn = 1;
	ceiling = Z_Malloc(data, sizeof(*ceiling), PU_LEVSPEC, 0);
